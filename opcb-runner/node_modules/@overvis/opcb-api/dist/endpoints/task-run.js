...
const OK_RESPONSE_SCHEMA = typebox_1.Type.Union([
    ...
    typebox_1.Type.Object({ result: typebox_1.Type.Literal("already-running") }),
...
async function openHandler(req) {
    const runningIds = (0, tasks_1.getRunningTaskIds)(this.memdb);
    if (runningIds.includes(req.params.id)) {
        return { result: "already-running" };
    }
    const res = await this.bus.sendRequest(new opcb_ts_shared_1.redisCmd.TaskRun(req.params.id));
    if (res.error) {
