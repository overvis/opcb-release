...
async function openHandler(req) {
    const testServerApiUrl = (await this.bus.getConfs(["overvisVpn:testServerApiUrl"]))["overvisVpn:testServerApiUrl"];
    if (!testServerApiUrl) {
        throw new Error("Motherland VPN IP is not set.");
    }
    const res = await axios_1.default.get(`${testServerApiUrl}/check-device-test/?testRunId=${req.query.testRunId}`, {
