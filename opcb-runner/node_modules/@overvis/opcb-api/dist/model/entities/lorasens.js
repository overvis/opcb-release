...
class QueryModel extends base_1.QueryModelBase {
    ...
    updateDeviceUpdatePeriodSec(args) {
        this.getQuery(`update lorasens_devices set update_period_sec = @updatePeriodSec where did = @did;`).run(args);
    }
    ...
    setDeviceActiveTill(args) {
        this.getQuery(`update lorasens_devices set active_mode_till = @activeTill where did = @did;`).run(args);
    }
    getDeviceByDid(did) {
    ...
    getDeviceStatus(did) {
        return (0, opcb_ts_shared_1.nullToUndefined)(this.getQuery(`select * from lorasens_device_statuses where did = @did;`, true).get({
            did,
        }));
    }
    getCommands(did) {
        return this.getQuery(`select * from lorasens_commands where did = @did and resulted_on is null order by created_on desc;`).all({
            did,
    ...
    getUnbindedDevices() {
        return this.getQuery(`select * from lorasens_unbinded_devices where blacklisted_on is null;`, true).all().map(opcb_ts_shared_1.nullToUndefined);
    }
    upsertRegLastGetRequestOn(args) {
        this.getQuery(`insert into lorasens_registers_requests (
            ...
            @registerAddress,
            @lastGetRequestOn
        ) on conflict(did, register_address) do update set last_get_request_on = excluded.last_get_request_on;`).run(args);
    }
    upsertRegisterValue(args) {
        this.getQuery(`insert into lorasens_registers_requests (
            ...
            @lastGetRequestOn,
            @value
        ) on conflict(did, register_address) do update set set_request_value = excluded.set_request_value;`).run(args);
    }
}
