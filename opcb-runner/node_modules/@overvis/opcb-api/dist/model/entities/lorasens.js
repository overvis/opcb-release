...
class QueryModel extends base_1.QueryModelBase {
    ...
    upsertRegLastGetRequestOn(args) {
        this.asTransaction(() => {
            for (const reg of args.registerAddress) {
                this.getQuery(`insert into lorasens_registers_requests (
                        ...
                        do update set last_get_request_on = excluded.last_get_request_on;`).run({
                    did: args.did,
                    registerAddress: reg,
                    lastGetRequestOn: args.lastGetRequestOn,
                });
            }
        });
    }
    upsertRegisterValue(args) {
        this.asTransaction(() => {
            for (const addr in args.registers) {
                this.getQuery(`insert into lorasens_registers_requests (
                        did,
                        register_address,
                        last_get_request_on,
                        ...
                        do update set set_request_value = excluded.set_request_value;`).run({
                    did: args.did,
                    registerAddress: addr,
                    lastGetRequestOn: args.lastGetRequestOn,
                    value: args.registers[addr],
                });
            }
        });
    }
