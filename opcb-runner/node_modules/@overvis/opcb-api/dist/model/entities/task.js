...
class QueryModel extends base_1.QueryModelBase {
    ...
    getRunning() {
        return this.getQuery(`select task_id from task_runs where finished_on is null;`, true).all();
    }
    insert(args) {
    ...
    getFiles() {
        return this.getQuery(`select * from task_files;`, true).all().map(opcb_ts_shared_1.nullToUndefined);
    }
    getRuns(taskId) {
        return this.getQuery(`select * from task_runs where task_id = @taskId order by started_on asc;`, true).all({ taskId }).map(opcb_ts_shared_1.nullToUndefined);
    }
    delete(taskId) {
