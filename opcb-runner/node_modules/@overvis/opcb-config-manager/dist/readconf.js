...
exports.validateAndFixConfig = exports.readConfigFile = void 0;
...
async function readConfigFile(configFile, factoryConfigFile) {
    ...
    try {
        ...
        _1.logger.warn(e, `Error retrieving or parsing JSON config file.`);
    ...
    const hasChangesFromFactory = (0, factory_1.mergeUserConfUponFactoryConf)(factoryConfJson, userConfJson);
    const confJson = factoryConfJson;
    return [confJson, hasChangesFromFactory];
}
exports.readConfigFile = readConfigFile;
async function validateAndFixConfig(confRecord, configFile, forceSave) {
    ...
    let lastErrorsNum;
    for (let i = 0; i < 10; i++) {
        validated = validate.Check(confRecord);
        if (validated) {
            break;
        }
        const errors = Array.from(validate.Errors(confRecord));
        _1.logger.warn(`Config file contains errors. Attempting to fix...`);
        const cleanErrors = extractErrors(errors);
        // TODO_FUTURE: Compare errors by content (as strings) instead of just counting the number of errors.
        ...
        for (const error of cleanErrors) {
            _1.logger.warn(`Config error: ${(0, logic_1.displayValidationError)(error, confRecord)}`);
            const schemaChanged = attemptToFixError(confRecord, error, opcb_ts_shared_1.CONFIG_SCHEMA);
            forceSave = true;
            if (schemaChanged) {
    ...
    const conf = confRecord;
    // save config to file
    if (forceSave) {
        _1.logger.warn(`Saving fixed config...`);
        await (0, write_1.saveConfigToFile)(conf, configFile);
    }
    return [conf, validate];
}
exports.validateAndFixConfig = validateAndFixConfig;
function extractErrors(errors) {
    var _a;
    const map = new Map();
    for (const err of errors) {
        let existingError = map.get(err.path);
        let type = (_a = existingError === null || existingError === void 0 ? void 0 : existingError.type) !== null && _a !== void 0 ? _a : "common";
