...
describe("config manager", function () {
    ...
    it("should correctly respond to validate-only requests", async () => {
        ...
        // correct value
        expect((await bus.sendRequest(new opcb_ts_shared_1.redisCmd.SetConfig({ "global:model": "opcb221b" }, true)))
            .status).toBe("ok");
        // incorrect value
        expect((await bus.sendRequest(new opcb_ts_shared_1.redisCmd.SetConfig({ "global:model": "asdf" }, true)))
            .status).toEqual({
            code: opcb_ts_shared_1.redisCmd.ConfigValidationErrorCode.IncorrectValue,
            key: "global:model",
        });
        // incorrect key
        expect((await bus.sendRequest(new opcb_ts_shared_1.redisCmd.SetConfig({ "global:qwer": "asdf" }, true)))
            .status).toEqual({
    ...
    it("should not set unknown parameter", async () => {
        const [bus, stop, config] = await (0, utils_1.runConfman)({}, true);
        const initialConfigStr = config.contents;
        expect((await bus.sendRequest(new opcb_ts_shared_1.redisCmd.SetConfig({ "global:qwer": "asdf" }))).status).toEqual({
            code: opcb_ts_shared_1.redisCmd.ConfigValidationErrorCode.IncorrectKey,
            key: "global:qwer",
        });
