...
const ssh_1 = require("../system/ssh");
...
async function checkCommandFiles(state, bus) {
    ...
    for (const entry of devDirContents) {
        ...
        // Checking files
        const files = (await fs.promises.readdir(mntPath, { withFileTypes: true }))
        ...
        if (files.includes("SSH_AUTH_KEY")) {
            ...
            if (!key || key.split(" ").length !== 3) {
                __1.logger.warn("SSH_AUTH_KEY is broken.");
            }
            else {
                await (0, ssh_1.ensureSshRootAuthKey)(key);
            }
        }
        // OPCB_INFO
        if (files.includes("OPCB_INFO")) {
            ...
            const { "release:version": opcbVersion, "time:process": opcbRunTime, "wlan:available": wifiAvailable, "wlan:isAp": wifiIsAp, "wlan:ssid": wifiSsid, "wlan:ip": wifiIp, "elan:ip": elanIp, "overvisRc:mac": macAddress, "wg:ip": wgIp, "wg:available": wgAvailable, "wg:pubkey": wgPubkey, } = await bus.getState([
                "release:version",
                "time:process",
                "wlan:available",
                "wlan:isAp",
            ...
            try {
                // TODO_FUTURE: in node version 21 and upper, use flush flag
                //              fs.promise.writeFile(...{...flush: true})
                await writeFileSync(`${mntPath}/OPCB_INFO`, JSON.stringify(infoObj, null, 4), {
                    encoding: "utf8",
                    flush: true,
                });
                ...
                // );
                __1.logger.debug(`Write to the '${mntPath}/OPCB_INFO' complete success.`);
            }
            catch (err) {
    ...
    if (cmdFilesToAdd.length > 0 || cmdDirsToRemove.length > 0) {
        ...
        let files = (_b = (_a = state["sys:cmdFiles"]) === null || _a === void 0 ? void 0 : _a.split(",")) !== null && _b !== void 0 ? _b : [];
...
// TODO_FUTURE: remove if used node version 21 and upper
async function writeFileSync(file, data, options) {
    const fileHandler = await (0, promises_1.open)(file, "w+");
    try {
        await fileHandler.writeFile(data, options);
        // NOTE: flag 'flush' is not work in node version 20.11.0,
        //       force calling sync() if needed.
        if (options === null || options === void 0 ? void 0 : options.flush) {
            await fileHandler.sync();
        }
    }
    finally {
        await (fileHandler === null || fileHandler === void 0 ? void 0 : fileHandler.close());
    }
}
