...
async function checkCommandFiles(state, bus) {
    ...
    for (const entry of devDirContents) {
        ...
        if (files.includes("OPCB_INFO")) {
            ...
            const conf = await bus.getConfs(["overvisVpn:pinCode"]);
            const state = await bus.getState([
                "release:version",
                "wlan:available",
                "wlan:isAp",
                "wlan:ssid",
            ...
            // Create info object
            const infoObj = {
                opcbVersion: state["release:version"],
                elanIp: state["elan:ip"],
                wifiMode: state["wlan:available"]
                    ? state["wlan:isAp"]
                        ? "ap"
                        : "state"
                    : "disabled",
                wifiSsid: state["wlan:ssid"],
                wlanIp: state["wlan:ip"],
                mac: state["overvisRc:mac"],
                wgIp: state["wg:ip"],
                pin: conf["overvisVpn:pinCode"],
                overvisUrl: conf["overvisVpn:pinCode"]
                    ? `http://c.overvis.com/${conf["overvisVpn:pinCode"]}`
                    : undefined,
            };
