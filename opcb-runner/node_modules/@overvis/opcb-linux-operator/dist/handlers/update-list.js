...
function createUpdateListHandler(bus) {
    return async (cmd) => {
        ...
        if (cmd.name === "netifs") {
            ...
            // NOTE: use option 'withFileTypes: true' for return name & path
            let dirents = await fs.promises.readdir("/dev", { withFileTypes: true });
            const ttyList = dirents
                .filter((e) => e.name.startsWith("ttyUSB"))
                .map((e) => `${e.path}/${e.name}`);
            let byIdList = [];
            try {
                dirents = await fs.promises.readdir("/dev/serial/by-id", { withFileTypes: true });
                byIdList = dirents.map((e) => `${e.path}/${e.name}`);
            }
            catch (e) {
                __1.logger.warn("Failed to read /dev/serial/by-id");
            }
            let byPathList = [];
            try {
                dirents = await fs.promises.readdir("/dev/serial/by-path", {
                    withFileTypes: true,
                });
                byPathList = dirents.map((e) => `${e.path}/${e.name}`);
            }
            catch (e) {
                __1.logger.warn("Failed to read /dev/serial/by-path");
            }
            let list = [...ttyList, ...byIdList, ...byPathList];
            if (list.length === 0) {
                list = ["/dev/ttyUSB0"];
            }
            await bus.setList(cmd.name, list);
        }
        else if (cmd.name === "sshkeys") {
            const keys = await (0, ssh_1.getSshRootAuthKeys)();
            await bus.setList(cmd.name, keys);
        }
        else {
