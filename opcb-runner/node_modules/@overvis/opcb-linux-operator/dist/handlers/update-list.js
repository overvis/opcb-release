...
function createUpdateListHandler(bus) {
    return async (cmd) => {
        ...
        if (cmd.name === "netifs") {
            // NOTE: os.networkInterfaces() does not return disabled connections
            ...
            const list = files.filter((f) => f.startsWith("ttyUSB") || f.startsWith("ttyS"));
            await bus.setList(cmd.name, list);
        }
        else if (cmd.name === "aps") {
...
async function getApsList(wlanNetif) {
    const [out, code] = await (0, exec_1.execCmd)("/usr/bin/timeout --signal=SIGKILL 15 /usr/bin/sh -c " +
    ...
    if (code === 124 || code === 137) {
        ...
        __1.logger.warn(`Failed to scan for access points: exit code ${code}`);
        return;
    }
    let apBlocks = [];
