...
async function getWireguardDump() {
    const [out, code] = await (0, exec_1.execCmd)("wg", ["show", "wg0", "dump"], "silent");
    if (code) {
        __1.logger.warn(`"wg show wg0 dump" exited with code ${code}: ${out}`);
...
async function updateWgInfo(state) {
    ...
    const wgIface = ifaces.wg0;
    if (wgIface) {
        for (const iface of wgIface) {
            if (iface.family === "IPv4") {
                wgIp = iface.address;
                wgMask = iface.netmask;
                break;
            }
        }
    }
    ...
    const wgPing = await (0, exec_1.ping)(state.settings.vpnMotherlandServerIp);
    return {
        "wg:available": true,
