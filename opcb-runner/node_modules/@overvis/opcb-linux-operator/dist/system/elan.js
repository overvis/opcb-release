...
async function ensureConnectionIsUp(name) {
    ...
    if (!(await connectionIsUp(name))) {
        ...
        const [out, exitCode] = await (0, exec_1.execCmd)("nmcli", ["connection", "up", name]);
        if (exitCode !== 0) {
            __1.logger.error(`Failed to activate connection ${name}: ${out}`);
        }
    }
}
...
async function updateConnection(name, iface, changes, vpnMotherlandServerIp, wait = 10) {
    ...
    __1.logger.info(`Connecting ${iface} device...`);
    const res = await (0, exec_1.execCmd)("nmcli", ["--wait", wait.toString(), "connection", "up", name]);
    if (res[1] !== 0) {
        __1.logger.warn(`"nmcli connection up" exited with code ${res[1] || "null"}`);
    }
