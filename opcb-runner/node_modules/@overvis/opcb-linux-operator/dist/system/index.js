...
async function ensureSystemState({ config, bgworker, staticFilesDir, labelFile, changelogFile, updateWlanConnection, }) {
    ...
    if (config.global.manageUsbModeSwitch) {
        await (0, usbmodeswitch_1.ensureUsbModeSwitchIsConfigured)();
    }
    if (config.global.manageNetworking) {
        ...
        if (config.gsm.manageGsm) {
            await (0, gsm_1.ensureGsmIsConfigured)(config.gsm);
        }
    }
    ...
    if (config.overvisVpn.isEnabled && config.overvisVpn.telemetry.isEnabled) {
        await (0, telemetry_1.ensureTmReportDiagnosticIsInstalled)(config.overvisVpn.telemetry.serverApiUrl, config.overvisVpn.telemetry.reportPeriodMinutes, {
            mac,
            fwVersion: config.global.lastRunVersion,
            vpnIpAddress: config.overvisVpn.wireguard.isManaged
                ? (_a = config.overvisVpn.wireguard.ipAddressCidr) === null || _a === void 0 ? void 0 : _a.split("/")[0]
                : undefined,
        });
    }
    else {
        await (0, telemetry_1.ensureTmReportDiagnosticIsUninstalled)();
    }
    ...
    if (config.overvisVpn.isEnabled && config.overvisVpn.addRootSshKey) {
        await (0, ssh_1.ensureSshRootAuthKey)(config.overvisVpn.addRootSshKey);
        // NOTE: setting value to empty string instead of null to prevent the key from being added on next boot
        changedConfigs["overvisVpn:addRootSshKey"] = "";
        __1.logger.info("Authorized root SSH key was set, removing 'overvisVpn:addRootSshKey' from the config.");
    }
    if (config.global.setupAutomaticFSCheckOnBoot) {
        await (0, fsck_1.ensureCheckDiskFileSystem)();
    }
    if (config.global.setupArmbianSpi) {
        await (0, spidev_1.ensureSpiDevIsEnabled)();
    }
    ...
    (0, bgworker_1.ensureBgWorkerJobs)(bgworker, {
        ...
        manageGsm: config.gsm.manageGsm,
