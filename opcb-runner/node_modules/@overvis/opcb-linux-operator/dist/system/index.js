...
async function ensureSystemState(config, monitor, bus, staticFilesDir) {
    ...
    if (config.global.manageNetworking) {
        ...
        await (0, wlan_1.ensureWlanIsConfigured)(config.wlan, config.overvisVpn.wireguard.vpnMotherlandServerIp, mac);
    }
    ...
    if (config.http.manageNginx) {
        await (0, nginx_1.ensureNginxIsConfigured)(config.http, staticFilesDir);
    }
    ...
    (0, monitor_1.ensureMonitoring)(monitor, config.monitoring, config.elan.networkInterfaceName, config.wlan.networkInterfaceName, config.overvisVpn.wireguard.vpnMotherlandServerIp, config.overvisVpn.wireguard.isManaged, config.global.manageNetworking, config.global.time.manageSettings, config.global.updateMode, config.global.autoUpdateTimeRange, config.global.allowForcedUpdates);
