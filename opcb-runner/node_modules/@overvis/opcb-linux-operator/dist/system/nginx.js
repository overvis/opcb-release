...
async function ensureNginxIsConfigured(config, staticFilesDir, labelFile, changelogFile) {
...
server {
    ...
    location /dyn-media/CHANGELOG.md {
        alias ${changelogFile};
        expires -1;
        add_header 'Cache-Control' 'no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0';
    }

    location /dyn-media/label.png {
        root /;
        try_files ${labelFile} ${staticFilesDir}/ui/media/opcb-label.png =404;
        expires -1;
        add_header 'Cache-Control' 'no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0';
    }
