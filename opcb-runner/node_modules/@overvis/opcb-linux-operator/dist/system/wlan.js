...
async function ensureWlanIsConfigured(config, vpnMotherlandServerIp, mac, updateWlanConnection) {
    ...
    if (options["802-11-wireless.ssid"] !== ssid) {
        if (config.mode === "ap") {
            __1.logger.info(`Changing AP SSID to "${ssid}".`);
        }
    ...
    if (config.mode === "station") {
        ...
        if (config.stationSettings.dhcpIsEnabled) {
            ...
            if (options["ipv4.addresses"] !==
                config.stationSettings.staticIpSettings.ipAddress + "/24") {
                changes["ipv4.addresses"] =
                    config.stationSettings.staticIpSettings.ipAddress + "/24";
            }
        ...
        if (options["ipv4.addresses"] !== config.apSettings.ipAddress + "/24") {
            changes["ipv4.addresses"] = config.apSettings.ipAddress + "/24";
        }
        if (options["ipv4.gateway"] !== "") {
            changes["ipv4.gateway"] = "";
        }
    ...
    const encPass = config.mode === "station" ? config.stationSettings.password : config.apSettings.password;
    if (encPass) {
        const pass = (0, opcb_ts_shared_1.decryptWifiPassword)(encPass);
    ...
    updateWlanConnection("opcb-wlan", config.networkInterfaceName, changes, vpnMotherlandServerIp, wait);
}
