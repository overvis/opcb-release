...
var IPv4WithPort = /** @class */ (function (_super) {
    tslib_1.__extends(IPv4WithPort, _super);
    function IPv4WithPort() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    IPv4WithPort.prototype.toModbusValue = function (redisVal) {
        if (redisVal === null) {
            return new Uint16Array(new Array(5).fill(0));
        }
        var arr = new Uint16Array(5);
        var _a = redisVal.split(":"), ip = _a[0], port = _a[1];
        var parts = ip.split(".");
        for (var i = 0; i < parts.length; i++) {
            arr[i] = Number(parts[i]);
        }
    ...
    IPv4WithPort.prototype.fromModbusValue = function (modbusVal) {
        var numbers = [];
        for (var i = 0; i < 5; i++) {
            var number = Number(modbusVal[i]);
            if (number < 0 || number > 255) {
        ...
        return "".concat(numbers.slice(0, 4).join("."), ":").concat(numbers[4]);
    };
    IPv4WithPort.prototype.getRegisterAddresses = function () {
        var _this = this;
        return Array.from(Array(5).keys()).map(function (i) { return _this.address + i; });
    };
    return IPv4WithPort;
}(ModbusMapper));
