...
var ModbusResponse = /** @class */ (function (_super) {
    ...
    ModbusResponse.fromMsg = function (msg) {
        ...
        return new ModbusResponse(msg.DATA, msg.TYPE.toString() === "0" ? "rtu-no-crc" : "as-is", errStr === "NONE" ? undefined : errStr);
    };
    ModbusResponse.prototype.toMsg = function () {
        var msg = { DATA: this.data };
        msg.TYPE = Buffer.from(this.type === "rtu-no-crc" ? "0" : "1");
...
function defaultProcessAsyncError(logger, err) {
    // TODO_FUTURE: sentry
    logger.fatal(err, "Error in bus message handler: ".concat(err.message));
...
var RedisBus = /** @class */ (function () {
    function RedisBus(connectString, name, logger, processAsyncError) {
        if (processAsyncError === void 0) { processAsyncError = function (err) {
            defaultProcessAsyncError(logger, err);
        }; }
        this.name = name;
        this.logger = logger;
                    ...
                    case 2:
                        ...
                        _loop_1 = function (message) {
                            ...
                            handler(deserializeObjFromMsg(msg_1))
                                ...
                                if (result !== undefined) {
                                    var respMsg = serializeObjToMsg(result, _this.name, msg_1.SRC, id);
                                    return _this.sendRawMsg(respMsg);
                                }
                                return undefined;
                            })
                                .catch(this_1.processAsyncError);
                        };
                        this_1 = this;
