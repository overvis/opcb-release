...
const lora_sens_1 = require("./lora-sens");
function createSyncConfigHandler(state) {
    return async () => {
        ...
        await state.bus.sendMsg(new opcb_ts_shared_1.redisCmd.Done());
    };
}
exports.createSyncConfigHandler = createSyncConfigHandler;
function createModbusRequestHandler(state, configMap, stateMap, virtParamsMap, loraSensRegisters) {
    return async (cmd) => {
        ...
        switch (func) {
            ...
            case modbus_1.ModbusFunctionCode.ReadHoldingRegisters: {
                res = await respondToReadRequest(state, cmd, configMap, stateMap, virtParamsMap, loraSensRegisters);
                break;
            }
            // eslint-disable-next-line @typescript-eslint/no-unsafe-enum-comparison
            case modbus_1.ModbusFunctionCode.ReadInputRegisters:
                res = await respondToReadRequest(state, cmd, configMap, stateMap, virtParamsMap, loraSensRegisters, true);
                break;
            // eslint-disable-next-line @typescript-eslint/no-unsafe-enum-comparison
            case modbus_1.ModbusFunctionCode.WriteSingleHolding:
                res = await respondToWriteSingleRegister(state, cmd, configMap, loraSensRegisters);
                break;
            // eslint-disable-next-line @typescript-eslint/no-unsafe-enum-comparison
            case modbus_1.ModbusFunctionCode.WriteMultipleHolding:
                res = await respondToWriteMultipleRegisters(state, cmd, configMap, virtParamsMap, loraSensRegisters);
                break;
            default:
        ...
        return new opcb_ts_shared_1.redisCmd.ModbusResponse(response, "rtu-no-crc");
    };
}
exports.createModbusRequestHandler = createModbusRequestHandler;
async function respondToReadRequest(state, cmd, configMap, stateMap, virtParamsMap, loraSensRegisters, inputRegisters = false) {
    ...
    else if (startAddress >= 11000 && startAddress + quantity < 12000) {
        return (0, lora_sens_1.readLoraSensParams)(state.db, loraSensRegisters, startAddress, quantity);
    }
...
async function respondToWriteSingleRegister(state, cmd, paramsMap, loraSensRegisters) {
    ...
    else if (address >= 11000 && address < 12000) {
        return (0, lora_sens_1.writeLoraSensRegisters)(state.db, loraSensRegisters, address, [cmd.data.readUInt16BE(4)], modbus_1.ModbusFunctionCode.WriteSingleHolding);
    }
    ...
    const validationResp = await state.bus.sendRequest(new opcb_ts_shared_1.redisCmd.SetConfig({ [param.path]: value }));
...
async function respondToWriteMultipleRegisters(state, cmd, paramsMap, virtParamsMap, loraSensRegisters) {
    const startAddress = cmd.data.readUint16BE(2);
    const quantity = cmd.data.readUint16BE(4);
    if (quantity > 125) {
    ...
    if (startAddress >= 10000 && startAddress + quantity < 11000) {
        ...
        const values = Array.from({ length: quantity }, (_, i) => cmd.data.readUInt16BE(7 + i * 2));
        return (0, lora_sens_1.writeLoraSensRegisters)(state.db, loraSensRegisters, startAddress, values, modbus_1.ModbusFunctionCode.WriteMultipleHolding);
    }
    ...
    const validationResp = await state.bus.sendRequest(new opcb_ts_shared_1.redisCmd.SetConfig(values));
    if (validationResp.status !== "ok") {
        return (0, modbus_1.createModbusError)(modbus_1.ModbusFunctionCode.WriteMultipleHolding, modbus_1.ModbusExceptionCode.IllegalDataValue);
    }
    const response = Buffer.alloc(5);
