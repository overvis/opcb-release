...
describe("virtual device", function () {
    it("should correctly read texts parameters", async () => {
        ...
        // Text - id: 1100
        expect(String.fromCharCode((await bus.sendRequest(new opcb_ts_shared_1.redisBus.ModbusRequest((0, utils_1.makeReadRequest)(111, 1100, 1, 0x03), "rtu-no-crc"))).data.readUint16BE(3))).toBe("e");
        const multipleRes = await bus.sendRequest(new opcb_ts_shared_1.redisBus.ModbusRequest((0, utils_1.makeReadRequest)(111, 1100, 4, 0x03), "rtu-no-crc"));
        let text = "";
    ...
    it("should correctly write single text parameter", async () => {
        const [bus, stop] = await (0, utils_1.runVirtDevice)(true);
        // Text - id: 1100
        await bus.sendMsg(new opcb_ts_shared_1.redisBus.ModbusRequest((0, utils_1.makeWriteSingleRequest)(111, 1100, "s".charCodeAt(0)), "rtu-no-crc"));
    ...
    it("should correctly write multiple text parameters", async () => {
        const [bus, stop] = await (0, utils_1.runVirtDevice)(true);
        // Text - id: 1100
        await bus.sendMsg(new opcb_ts_shared_1.redisBus.ModbusRequest((0, utils_1.makeWriteMultipleRequest)(111, 1100, [
            "a".charCodeAt(0),
            "b".charCodeAt(0),
            "c".charCodeAt(0),
        ]), "rtu-no-crc"));
