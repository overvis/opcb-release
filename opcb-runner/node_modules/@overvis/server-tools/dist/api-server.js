...
function getApiServerConfigs(publicConfigSchema, secretConfigSchema) {
    (0, formats_1.configureTypeboxFormats)();
...
async function initApiFastifyServer(name, logger, decorations, enforceContentType = true) {
    const server = (0, fastify_1.default)({ logger, disableRequestLogging: true });
    ...
    server.addHook("onRequest", (req, _reply, done) => {
        req.receivedOn = Date.now();
        req.log.debug({ req: req.raw }, "received request");
        done();
    });
    server.addHook("onResponse", (req, reply, done) => {
        req.log.debug({ statusCode: reply.raw.statusCode }, "request completed");
        done();
    });
