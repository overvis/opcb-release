...
#
motherlandSshPublicKey=$(getJsonValue "motherlandSshPublicKey" "$response")
((err_mask|=$?)) || true
...
#
pinCode=$(getJsonValue "pinCode" "$response")
((err_mask|=$?)) || true
echo "  @ pinCode [ ${pinCode} ]"
#
internalModelName=$(getJsonValue "internalModelName" "$response")
((err_mask|=$?)) || true
echo "  @ internalModelName [ ${internalModelName} ]"
#
internalSolutionName=$(getJsonValue "internalSolutionName" "$response")
((err_mask|=$?)) || true
echo "  @ internalSolutionName [ ${internalSolutionName} ]"
#
manufacturerName=$(getJsonValue "manufacturerName" "$response")
((err_mask|=$?)) || true
echo "  @ manufacturerName [ ${manufacturerName} ]"
#
labelName=$(getJsonValue "labelName" "$response")
((err_mask|=$?)) || true
...
# 7. Add authorized key
echo ""
echo "7. Add 'motherlandSshPublicKey' to 'authorized_keys' list..."
umask 0077
...
# 9. Save pin code to the file
echo ""
echo "9. Save pin code to the 'dev-pin-code' file..."
umask 0077
